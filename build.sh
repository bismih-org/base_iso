...
chroot kaynak apt install ca-certificates -y
...
chroot kaynak /usr/bin/env DEBIAN_FRONTEND=noninteractive apt install -t yirmiuc-backports linux-image-amd64 -y

chroot kaynak apt install -y locales
chroot kaynak locale-gen en_US.UTF-8
chroot kaynak locale-gen tr_TR.UTF-8
chroot kaynak update-locale LANG=tr_TR.UTF-8

### grub paketleri için
chroot kaynak /usr/bin/env DEBIAN_FRONTEND=noninteractive apt install grub-pc-bin grub-efi-ia32-bin grub-efi -y

### live paketleri için -> dili otomatik seçtir
chroot kaynak /usr/bin/env DEBIAN_FRONTEND=noninteractive apt install live-config live-boot -y

echo "firmware-ipw2x00 firmware-ipw2x00/license/accepted select true" | chroot kaynak debconf-set-selections
echo "firmware-ivtv firmware-ivtv/license/accepted boolean true" | chroot kaynak debconf-set-selections

### firmware paketleri -> driverlede gen sorulara otomatik kabul ettir
chroot kaynak /usr/bin/env DEBIAN_FRONTEND=noninteractive apt install bluez-firmware firmware-amd-graphics firmware-atheros firmware-b43-installer firmware-b43legacy-installer firmware-bnx2 firmware-bnx2x firmware-brcm80211 firmware-cavium firmware-intel-sound firmware-ipw2x00 firmware-ivtv firmware-iwlwifi firmware-libertas firmware-linux firmware-linux-free firmware-linux-nonfree firmware-misc-nonfree firmware-myricom firmware-netxen firmware-qlogic firmware-realtek firmware-samsung firmware-siano firmware-ti-connectivity firmware-zd1211 firmware-sof-signed zd1211-firmware -y
chroot kaynak update-initramfs -u


### kde paketleri
chroot kaynak /usr/bin/env DEBIAN_FRONTEND=noninteractive apt install kde-standard -y
...
chroot kaynak apt purge juk kmail* konqueror kwrite kde-spectacle zutty -y
